...
auto-mode-onboarding, the fullscreen-renderer recovery dialog) Drovr can
press itself, an `mcp-approval`/tool-`permission` prompt with its own
existing, deliberately unautomated flow, or a genuinely `unknown` dialog.
...
the full contract, an example, and this release's fullscreen-renderer
caveats.
